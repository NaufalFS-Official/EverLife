...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| **BUG-001** | P3 | Open | UX / Feedback | F-004 / AC-003 | Feedback visual/audio ketiadaan respon saat tap dock navigasi bawah ketika modal skenario naratif sedang aktif | Guard state memblokir transisi ke `SUBMENU_OPEN`, namun tidak ada toast/efek goyang pada dialog modal untuk memberi tahu pemain bahwa peristiwa saat ini wajib diselesaikan | Menunggu sesi /fix berikutnya | `tests/playtest/structured_playtest.test.ts` |
| **BUG-002** | P3 | Open | Hygiene / Memory | F-009 / AC-008 | `setTimeout` reload pasca-impor save di `SettingsModal.tsx` beroperasi tanpa timer cleanup saat unmount | Pemanggilan `setTimeout(platform.reload, 1000)` tidak menyimpan reference ID timer ke state/ref untuk dibersihkan via `clearTimeout` bila modal ditutup seketika | Menunggu sesi /fix berikutnya | `tests/unit/storage_fallback.test.ts` |
| **BUG-003** | P2 | Open | Tooling / E2E | S15 / AC-008 | Eksekusi Playwright headless terhenti di lingkungan lokal CLI karena binary Chromium (`chrome-headless-shell.exe`) belum diunduh | Direktif D20 melarang pengunduhan binary eksternal otomatis tanpa konfirmasi pengguna (`npx playwright install`) | Menunggu sesi /fix berikutnya | `tests/e2e/smoke.spec.ts` |
